...
export class AuthenticationService {
    ...
    interceptUrl(req: HttpRequest<any>): boolean {
        return req.url.startsWith(environment.BACKEND_BASE_URL)
        && !req.url.startsWith(environment.BACKEND_BASE_URL)
    ...
    private loadAccessToken(retrieveAccessToken: boolean, refreshToken?: string, username?: string, password?: string):
        ...
        return this.http.post<any>(environment.loginUrl, params,
            {
                headers: corsHeaders
            }
        ).pipe(
